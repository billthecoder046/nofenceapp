...
class CrimeBloc extends ChangeNotifier {
  ...
  Future<void> createCrime(Crime newCrime) async {
    try {
      ...
      final docRef =
          FirebaseFirestore.instance.collection(FirebaseConfig.crimesCollection).doc();
  ...
  Future<void> fetchAllCrimes({bool refresh = false}) async {
    ...
    if (rawData.docs.length > 0) {
      ...
      crimes.addAll(rawData.docs
          .map((doc) => Crime.fromJSON(doc.data() as Map<String, dynamic>))
          .toList());
  ...
  Future<void> addCriminalToCrime(String crimeId, String criminalId,
      {required CriminalBloc criminalBloc}) async {
    try {
      final docRef = FirebaseFirestore.instance
          .collection(FirebaseConfig.crimesCollection)
          .doc(crimeId);
  ...
  Future<List<Crime>> getCrimesByLocation(
      double latitude, double longitude, double radius, context) async {
    try {
      final geoFirestore =
          Provider.of<GeoFirestore>(context, listen: false); // Get GeoFirestore instance
      ...
      return documents
          .map((document) => Crime.fromJSON(document.data() as Map<String, dynamic>))
          .toList();
    } catch (e) {
  ...
  Future<void> setCrimeLocation(
      String crimeId, double latitude, double longitude, context) async {
    try {
      final geoFirestore =
          Provider.of<GeoFirestore>(context, listen: false); // Get GeoFirestore instance
  ...
  Future<void> removeCriminalFromCrime(String crimeId, String criminalId,
      {required CriminalBloc criminalBloc}) async {
    try {
      final docRef = FirebaseFirestore.instance
          .collection(FirebaseConfig.crimesCollection)
